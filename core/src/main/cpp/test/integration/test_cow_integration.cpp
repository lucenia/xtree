...
TEST_F(COWIntegrationTest, BasicCOWFunctionality) {
    // Create COW-enabled index with FILE_IO mode (MMAP now uses CompactAllocator)
    IndexDetails<DataRecord>* index = new IndexDetails<DataRecord>(
        2, 32, dimLabels, nullptr, nullptr, 
        IndexDetails<DataRecord>::PersistenceMode::FILE_IO, "test_cow.snapshot"  // Enable COW with FILE_IO
    );
...
TEST_F(COWIntegrationTest, CompareWithAndWithoutCOW) {
    ...
    IndexDetails<DataRecord>* idx2 = new IndexDetails<DataRecord>(
        2, 32, dimLabels, nullptr, nullptr,
        IndexDetails<DataRecord>::PersistenceMode::FILE_IO, "test_cow.snapshot");  // Enable COW
...
TEST_F(COWIntegrationTest, SnapshotValidation) {
    // Create index with COW using FILE_IO mode for traditional snapshot validation
    IndexDetails<DataRecord>* index = new IndexDetails<DataRecord>(
        2, 32, dimLabels, nullptr, nullptr,
        IndexDetails<DataRecord>::PersistenceMode::FILE_IO, "test_cow.snapshot");
    ...
    // Validate snapshot - only works with traditional backend
    if (index->getCOWManager()->getBackendType() == DirectMemoryCOWManager<DataRecord>::BackendType::TRADITIONAL) {
...
TEST_F(COWIntegrationTest, AllocatorUsage) {
    ...
    // Without COW
    {
        IndexDetails<DataRecord> index(2, 32, dimLabels, nullptr, nullptr);
