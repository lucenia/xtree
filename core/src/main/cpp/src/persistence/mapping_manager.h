...
public:
    ...
    public:
        Pin() = default;
        Pin(MappingManager* mgr, FileMapping* fmap, MappingExtent* ext, uint8_t* ptr)
            : mgr_(mgr), fmap_(fmap), ext_(ext), ptr_(ptr) {}
        
        // Move-only
        Pin(Pin&& other) noexcept
            : mgr_(other.mgr_), fmap_(other.fmap_), ext_(other.ext_), ptr_(other.ptr_) {
            other.mgr_ = nullptr;
            other.fmap_ = nullptr;
            other.ext_ = nullptr;
            other.ptr_ = nullptr;
